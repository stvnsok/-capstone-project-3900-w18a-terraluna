...
| **/auth/register** | POST | Register a new user with a username, email and password. Generates a new access/refresh token pair. | JSON<pre>{<br>    "username": "James",<br>    "email": "james@test.com",<br>    "password": "Password123"<br>}</pre> | <pre>201<br>{<br>    "access_token": <access_token>,<br>    "refresh_token": <refresh_token>,<br>    "username": "James"<br>}</pre> | <pre>422 Invalid username format<br>422 Invalid email format<br>422 Password too weak<br>409 Username is in use<br>409 Email is in use</pre> |
| **/auth/login** | GET | Get username of logged in user. | Args<pre>?access_token=<access_token></pre> | <pre>200<br>{"username": "James"}</pre> | |
| **/auth/login** | POST | Log in a user with valid credentials. | JSON<pre>{<br>    "username_or_email": "james@test.com",<br>    "password": "Password123"<br>}</pre> | <pre>200<br>{<br>    "access_token": <access_token>,<br>    "refresh_token": <refresh_token>,<br>    "username": "James"<br>}</pre> | <pre>401 Incorrect username/email and/or password</pre> |
| **/auth/logout** | DELETE | Log out a user by revoking their access token. | | <pre>204<br>""</pre> | |
| **/auth/reset/username** | PUT | Reset a user's username to a valid unique username. Fresh access token required. | JSON<pre>{"new_username": "David"}</pre> | <pre>200<br>{"username": "David"}</pre> | <pre>422 Invalid username format<br>409 Username is in use</pre> |
| **/auth/reset/email** | PUT | Reset a user's email to a valid unique email. Fresh access token required. | JSON<pre>{"new_email": "david@test.com"}</pre> | <pre>204<br>""</pre> | <pre>422 Invalid email format<br>409 Email is in use</pre> |
| **/auth/reset/password** | PUT | Reset a user's password to a valid password. Fresh access token required. | JSON<pre>{<br>    "old_password": "Password123",<br>    "new_password": "Password12345"<br>}</pre> | <pre>204<br>""</pre> | <pre>401 Incorrect password<br>422 Password too weak</pre> |
...
| **/my_recipes/{id}/publish** | PUT | Publish a recipe. Recipes cannot be published unless all entries are completed, except videos per instruction are optional. | | <pre>200<br>{<br>    "recipe": {<br>        "id": 1,<br>        "status": "Draft",<br>        "name": "Pasta",<br>        "cookTime": 50,<br>        "mealType": ["lunch"],<br>        "dietType": ["vegan"],<br>        "description": "Pasta recipe",<br>        "imageUrl": "/home/bob/images/cat.jpg",<br>        "ingredients": [<br>            {<br>                "id": 1,<br>                "name": "water",<br>                "units": "ml",<br>                "quantity": 5<br>            },<br>            ...<br>        ],<br>        "steps": [<br>            {<br>                "instruction": "Mix water and flour",<br>                "videoUrl": "home/bob/videos/water.mp4"<br>            },<br>            ...<br>        ],<br>        "reviews": [<br>            {<br>                "stars": 5,<br>                "review": "very good"<br>            },<br>            ...<br>        ]<br>    }<br>}</pre> | <pre>422 You cannot publish a copied recipe<br>422 Cannot perform this action until recipe is complete</pre> |
